...
/*
void quat2euler(float quat[], double* alpha, double* beta, double* gamma)
{
	*alpha = atan2(2*(quat[0]*quat[1]+quat[2]*quat[3]),1-2*(quat[1]*quat[1]+quat[2]*quat[2]));
	*beta = asin(2*(quat[0]*quat[2]-quat[3]*quat[1]));
	*gamma = atan2(2*(quat[0]*quat[3]+quat[1]*quat[2]), 1-2*(quat[2]*quat[2]+quat[3]*quat[3]));
}
*/
...
Matrix jacobian(string arm_ID,double L1, double L2, double L4, double L6, double theta1, double theta2, double theta3, double theta4, double theta5, double theta6, double theta7, double d1, double d2, double d3, double d4, double d5, double d6)
{
	if (arm_ID == "LEFT")
	{
		Matrix Jo(6,7);
...
Matrix kinematics(string arm_ID, double L1, double L2, double L4, double L6, double theta1, double theta2, double theta3, double theta4, double theta5, double theta6, double theta7, double d1, double d2, double d3, double d4, double d5, double d6)
{
	if (arm_ID == "LEFT")
	{
...
/*
Matrix TM_goal(double x, double y, double z, double a, double B, double r)
{
	Matrix Tg(4,4);

	Tg(0,0) = cos(B)*cos(a);
	Tg(0,1) = sin(B)*cos(a)*sin(r) - sin(a)*cos(r);
	Tg(0,2) = sin(a)*sin(r) + sin(B)*cos(a)*cos(r);
	Tg(0,3) = x;

	Tg(1,0) = cos(B)*sin(a);
	Tg(1,1) = cos(a)*cos(r) + sin(B)*sin(a)*sin(r);
	Tg(1,2) = sin(B)*sin(a)*cos(r) - cos(a)*sin(r);
	Tg(1,3) = y;
...
Matrix inversekinematics(string arm_ID, double goal[], Matrix rotation, double theta[], double velocity, double wo, double ko, double& _x7, double& _y7, double& _z7, double& _alpha, double& _beta, double& _gamma, double& _mm, vector<double>& joint)
{
    // Link length (cm)
	double L1=23;
    double L2=36.44;
...
//        dHo(j,0) = dHoo(j,0);
	...
	// The inverse of the diagonal weight matrix:
	dia=diag(W); //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	// eq5.38 in P96 of "Maximizing manipulation capabilities of persons with disabilities"
	...
	// from eq. 9.79 page 268 of Nakamura's book.
	*/
...
int main()
{
	...
	string left_arm_ID = "LEFT";
	string right_arm_ID = "RIGHT";
	...
	double _velocity = 0.6;				//////////////////////////////parameter
	double _ko = 250;
	double _wo = 5000;
	double _mm;
	...
	Client client("192.168.2.166");//<<===========================IP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
//	client.Recv(recvMsg, DEFAULT_BUFFER_LENGTH);
	...
	{	
		...
		{
			goal_left[0] = ((-(double)acd.controllers[0].pos[2]/10 + 100)*0.6 - trigger_x_left)*1 + trigger_arm_x_left;
			goal_left[1] = (-(double)acd.controllers[0].pos[0]/10*0.6 - trigger_y_left)*1 + trigger_arm_y_left;
			goal_left[2] = (((double)acd.controllers[0].pos[1]/10+50)*0.6 - trigger_z_left)*1 + trigger_arm_z_left;
		}
		...
		{
			goal_right[0] = ((-(double)acd.controllers[1].pos[2]/10 + 100)*0.6 - trigger_x_right)*0.3 + trigger_arm_x_right;
			goal_right[1] = (-(double)acd.controllers[1].pos[0]/10*0.6 - trigger_y_right)*0.3 + trigger_arm_y_right;
			goal_right[2] = (((double)acd.controllers[1].pos[1]/10+50)*0.6 - trigger_z_right)*0.3 + trigger_arm_z_right;
		}
		...
		if(acd.controllers[1].buttons == 128)
		{
			...
			goal_left[0] = ((-(double)acd.controllers[1].pos[2]/10 + 100)*0.6 - trigger_x_right)*1 + trigger_arm_x_left;
			goal_left[1] = (-(double)acd.controllers[1].pos[0]/10*0.6 - trigger_y_right)*1 + trigger_arm_y_left - 0.5;
			goal_left[2] = (((double)acd.controllers[1].pos[1]/10+50)*0.6 - trigger_z_right)*1 + trigger_arm_z_left;
			goal_right[0] = ((-(double)acd.controllers[1].pos[2]/10 + 100)*0.6 - trigger_x_right)*1 + trigger_arm_x_right;
			goal_right[1] = (-(double)acd.controllers[1].pos[0]/10*0.6 - trigger_y_right)*1 + trigger_arm_y_right + 0.5;
			goal_right[2] = (((double)acd.controllers[1].pos[1]/10 + 50)*0.6 - trigger_z_right)*1 + trigger_arm_z_right;
		...
		//cout<<goal[0]<<setw(10)<<goal[1]<<setw(10)<<goal[2]<<setw(10)<<_x7_right<<setw(10)<<_y7_right<<setw(10)<<_z7_right<<setw(7)<<_alpha_right<<setw(7)<<_beta_right<<setw(7)<<_gamma_right<<"\r";
		cout<<goal_right[0]<<setw(12)<<goal_right[1]<<setw(12)<<goal_right[2]<<setw(12)<<_alpha_right<<setw(12)<<_beta_right<<setw(12)<<_gamma_right<<"\r";
